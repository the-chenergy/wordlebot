...
const NORMAL = 0,
  HARD = 1;
// list size constants
const CHECK_SIZE = Infinity,
  TOP_TEN_LENGTH = +urlParams.get('t') || 120,
  MAX_TIME = Infinity;
// misc constants
const NOT_YET_TESTED = 0.999,
  SIZE_FACTOR = 5,
  INFINITY = 9999999;
...
function setLength() {
  ...
  document
    .getElementById('word-entered')
    .setAttribute('maxlength', word_length);
...
function getBestOf(list) {
  ...
  if (list[bot.type]) {
    ...
    best_list = best_list.map((a) =>
      Object.assign(
        {},
        { word: a.word, average: a[wordbank].average, wrong: a[wordbank].wrong }
      )
    );
    return best_list;
  }
...
function dontNeedToCheck(answers, unique_answers) {
  return (
    (answers.length <= 2 && !bot.isFor(ANTI) && bot.getCount() == 1) ||
...
function getPotentialGuessesAndAnswers() {
  ...
  let sorted_guess_list = initialGuesses(
    answer_list,
    sorted_answer_list,
    unique_answers,
...
function initialGuesses(
  ...
  sorted_guess_list = reduceListSize(
    sorted_guess_list,
    sorted_answer_list,
    bot.getAnswerListLength(answer_list)
  );
...
function updateLists(likely_answers, unlikely_answers, best_guesses) {
  ...
  updateHeaders(
    bot.getAnswerListLength(likely_answers),
    bot.getAnswerListLength(unlikely_answers)
  );
...
function writeBestGuessList(guesses) {
  ...
  for (let i = 0; i < list_length; i++) {
    ...
    if (!notFullyTested(guesses[i])) {
      ...
      if (common_words.includes(guesses[i].word)) {
        // console.log('common best guess', guesses[i]);
        dump += ', ' + guesses[i].word;
      }
    }

    let list_item = createListItem(
      guesses[i].word + (common_words.includes(guesses[i].word) ? '*' : ''),
      data,
      i + 1
    );
    list.push(list_item);
...
function notFullyTested(guess) {
  return (
    guess.wrong == NOT_YET_TESTED || bot.getCount() > 1 || bot.isFor(SPOTLE)
  );
}

function createListItem(word, data, rank) {
  let suggestion = createElement('span', word, 'click');
  let word_with_ranking = createElement(
    'div',
    rank + '. ' + suggestion.outerHTML,
    'suggestion'
  );
  ...
  let list_item = createElement(
    'li',
    word_with_ranking.outerHTML + score.outerHTML
  );
  return list_item;
}
...
function updateHeaders(likely_length, unlikely_length) {
  ...
  let subheading = document.getElementsByClassName(
  ...
  let likely_html =
    likely_length + ' probable answer' + pluralOrSingle(likely_length, '', 's');
  let unlikely_html =
    unlikely_length +
    ' unlikley possibilit' +
    pluralOrSingle(unlikely_length, 'y', 'ies');
  ...
  heading.innerHTML =
    total_length + ' possibilit' + pluralOrSingle(total_length, 'y', 'ies');
  subheading.innerHTML =
    likely_span.outerHTML + ', ' + unlikely_span.outerHTML + '.';
}
...
const NO_WORDS_LEFT_MESSAGE =
  "it doesn't look like we have this word. double check to make sure you all the clues you entered are correct.";
...
function unfoundAnswersMessage(unfound_answers) {
  let text =
    unfound_answers.length +
    ' of the answers ' +
    pluralOrSingle(unfound_answers.length, 'is ', 'are ');

  for (let i = 0; i < unfound_answers.length; i++) {
    let answer = createElement(
      'span',
      printAnswer(unfound_answers[i]),
      'final'
    );
...
function showFinalOptions(sorted, less_likely) {
  ...
  if (bot.getAnswerListLength(sorted)) {
    ...
    final_words.innerHTML =
      'The word is almost certainly ' + first_answer.outerHTML;

    if (sorted.length == 2) {
      let second_answer = createElement(
        'span',
        printAnswer(sorted[1]),
        'final'
      );
  ...
  if (bot.getAnswerListLength(less_likely)) {
    ...
    for (let i = 0; i < less_likely.length; i++) {
      let unlikely_answer = createElement(
        'span',
        printAnswer(less_likely[i]),
        'final'
      );
      unlikely.innerHTML += unlikely_answer.outerHTML;

      i < less_likely.length - 1
        ? (unlikely.innerHTML += ', ')
        : (unlikely.innerHTML += '.');
    }
...
function addToSlides(heading_html, suggestions) {
  ...
  let list = document
    .getElementsByClassName('best-guesses')[0]
...
function guessesMadeSoFar() {
  ...
  return Math.ceil(
    document.getElementsByClassName('row').length / bot.getCount()
  );
}
...
function makeTables(val, c) {
  ...
  if (val) {
    if (
      bot.isFor(SPOTLE) &&
      numberOfGuessesMadeIs() < 6 &&
      val != ' '.repeat(word_length)
    ) {
      replaceRow(guessesMadeSoFar(), val);
    } else {
  ...
  if ((numberOfGuessesMadeIs(1) && c == 'normal') || bot.isFor(SPOTLE)) {
    addButtons();
...
function replaceRow(num, word) {
  let row = document.getElementsByClassName('row')[num];
...
function makeWoodleDropdowns() {
  ...
  let wrong_spot_count = createElement(
    'select',
    '',
    'woodle-count ' + WRONG_SPOT
  );
...
function guessesArePrecomputed() {
  ...
  let hash = makeHash(
    ...
    wordbank,
    bot.guessesAllowed(),
    document.getElementsByClassName('warmle-selector')[0]?.value,
    diff
  );
...
function getBestGuesses(answer_list, guess_list, unique_answers) {
  ...
  // if (numberOfGuessesMadeIs(0)) {
  //   guess_list = ["LEAST", "CRANE", "TRADE", "TRANS", "STARE", "LEARN", "SNARE", "SLEPT", "LATER", "TAPER", "STORE", "SPELT", "SCARE", "SMART", "RENAL", "SNARL", "TRAMP", "STRAP", "CRAMP", "CLAMP", "CRIMP", "SCAMP"]; /* prettier-ignore */
  //   guess_list = ['STARE']; //, 'LEAST', 'CRANE', 'TRADE'];
  ...
  // console.log('initial_guesses', initial_guesses);

  if (bot.getCount() > 1 || bot.isFor(SPOTLE)) {
    best_guesses = initial_guesses;
  } else {
    best_guesses = calculateGuessList(
      unique_answers,
      guess_list,
      initial_guesses
    );
...
function removeUselessGuesses(list, possibilities) {
  ...
  for (let i = 0; i < list.length; i++) {
    for (let j = 0; j < word_length; j++) {
      ...
      if (
        alphabet[c][word_length] == 0 || // if letter isn't in any of the words
        (alphabet[c][j] == 0 &&
          alphabet[c][word_length] == possibilities.length)
      ) {
        // if letter isn't in that spot in any word
        list.splice(i, 1);
        i--;
        break;
      }
...
function getFirstGuesses() {
  ...
  first_guesses = getBestOf(first_guesses).filter(
    (a) => a.word.length == word_length
  );
...
function getTempList(guesses, answers) {
  ...
  guesses = guesses.map((a) =>
    Object.assign(
      {},
      { word: a.word, average: a.adjusted, wrong: NOT_YET_TESTED }
    )
  );
  return guesses;
}
...
function calculateGuessList(answers, guesses, best_words) {
  ...
  for (let i = 0; i < best_words.length; i++) {
    ...
    Object.keys(remaining).forEach(function (key) {
      // console.log('bwi', best_words[i].word, [...best_words[i].results['w']]);
      countResults(
        best_words[i],
        remaining[key],
        guesses,
    ...
    // if (0 && shouldStopTesting(start_time, performance.now(), can_finish, i)) {
    //   console.log(
    //     'only calculated ' +
    //       (i + 1) +
    //       ' words in ' +
    //       ((performance.now() - start_time) / 1000).toFixed(3) +
    //       ' seconds'
    //   );
  ...
  return best_words
    .map((a) =>
      Object.assign({}, { word: a.word, average: a.average, wrong: a.wrong })
    )
    .slice(0, TOP_TEN_LENGTH);
}
...
function getNextGuesses(new_guesses, answers, best, differences) {
  ...
  if (isDifficulty(HARD)) {
    ...
    list = reduceListSize(
      new_guesses,
      uniqueWordsFrom(answers),
      answers.length
    );
  } else {
    list = filterList(
      new_guesses,
      { word: best.word, colors: differences },
      true
    );
  }
...
function countResults(best, answers, guesses, results, attempt, differences) {
  ...
  if (
    answers.length <= 2 &&
    (!bot.isFor(ANTI) ||
      new_guesses.length == answers.length ||
      !answers.length)
  ) {
...
function filterList(list, letters, reduced_filter, split) {
  ...
  for (let guess = 0; guess < guessesMadeSoFar(); guess++) {
    list = createFilteredList(
      list,
      getWord(guess),
      bot.getRowColor(guess),
      reduced_filter,
      split,
      guess
    );
...
function xordleFilter(list) {
  ...
  for (let i = 0; i < list.length; i++) {
    let rest = list
      .slice(i + 1)
      .filter(
        (a) => bot.getDifference(list[i], a) == INCORRECT.repeat(word_length)
      );
...
function sortByWrongThenAverage(guesses) {
  guesses.sort(function (a, b) {
    ...
    if (bot.isBetter(a.average, b.average)) {
      return -1;
    }
    if (!bot.isBetter(a.average, b.average)) {
      return 1;
